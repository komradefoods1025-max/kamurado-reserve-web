...
function restoreSessionFromPending(pending) {
  ...
  session.currentSelection = safeJsonParse(row.currentSelectionJson, null);
...
async function handleSelectedDate(replyToken, userId, session, selectedDate) {
  ...
  if (!isYmdDate(normalizedDate)) {
    await savePendingSession(userId, currentSession);
    await replyMessage(replyToken, [
      textMessage('日付の形式が正しくありません。もう一度お選びください。'),
      createDateSelectMessage()
    ]);
    return;
  }
  ...
  if (!availableTimes.length) {
    await savePendingSession(userId, currentSession);
    await replyMessage(replyToken, [
      textMessage('この日は選べる受取時間がありません。別の日付をお選びください。'),
      createDateSelectMessage()
    ]);
    return;
  }
  ...
  if (
    currentSession.flowType === 'change' ||
    currentSession.step === 'change_waiting_date'
  ) {
    transitionSession(currentSession, 'change_waiting_time', { date: normalizedDate });
    await savePendingSession(userId, currentSession);
    await replyMessage(replyToken, [
      textMessage(`変更後の受取日：${formatDateWithWeekday(normalizedDate)}`),
  ...
  await savePendingSession(userId, currentSession);
  await replyMessage(replyToken, [
    textMessage(`受取日：${formatDateWithWeekday(normalizedDate)}`),
    buildTimeMessage(normalizedDate)
  ]);
}
...
async function getReminderTargets() {
  try {
    const url = buildReservationApiUrl({
      ...
      minutes: String(PENDING_REMINDER_MINUTES)
    });
    const response = await fetch(url);
    return await response.json();
  } catch (err) {
    console.error('getReminderTargets error:', err);
...
async function runPendingReminderJob() {
  ...
  const reminderResult = await getReminderTargets();
  const targets = Array.isArray(reminderResult?.targets)
    ? reminderResult.targets
    : [];
